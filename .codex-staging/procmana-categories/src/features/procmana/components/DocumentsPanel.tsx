...
type DocumentDialog = "menu" | "folder" | "vault";
...
export function DocumentsPanel({ project, estimates, canEdit, demo }: Props) {
  ...
  return <div className="space-y-6">
    ...
    {selectedDocument && documentDialog && <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/45 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closeDocumentDialog(); }}>
      <div role="dialog" aria-modal="true" aria-label={`${selectedDocument.name}の操作`} className="w-full max-w-md overflow-hidden rounded-[28px] bg-white p-6 shadow-2xl">
        ...
        {!busy.startsWith("document-") && documentDialog === "menu" && <div className="mt-6 space-y-2">
          <button type="button" className="flex w-full items-center gap-3 rounded-2xl bg-slate-50 px-4 py-4 text-left font-bold text-slate-800 hover:bg-blue-50 hover:text-blue-800" onClick={() => selectedDocument.fileUrl && window.open(selectedDocument.fileUrl, "_blank", "noopener,noreferrer")}><ZoomIn size={21} />プレビュー</button>
