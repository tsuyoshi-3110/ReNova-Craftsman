...
type BusinessDocumentCategory = Extract<ProjectDocumentCategory, "estimate" | "contract" | "order" | "invoice">;
...
const BUSINESS_DOCUMENT_GROUPS: ReadonlyArray<{
  category: BusinessDocumentCategory;
  description: string;
}> = [
  { category: "estimate", description: "提出前の下書きから採用済み見積まで" },
  { category: "contract", description: "契約確定・解除の履歴" },
  { category: "order", description: "業者別に確定した発注書" },
  { category: "invoice", description: "発行済みの請求書" },
];
  ...
  return <div className="space-y-6">
    ...
    <section className="pm-card overflow-hidden p-0">
      ...
      {generatedDocuments.length === 0
        ...
        : <div className="space-y-5 p-5">{BUSINESS_DOCUMENT_GROUPS.map((group) => {
          const groupDocuments = generatedDocuments.filter((document) => document.category === group.category);
          return <section key={group.category} aria-labelledby={`business-documents-${group.category}`} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3">
                <span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${CATEGORY_STYLES[group.category]}`}><FileText size={19} /></span>
                <div><h3 id={`business-documents-${group.category}`} className="font-bold text-slate-900">{CATEGORY_LABELS[group.category]}</h3><p className="mt-0.5 text-xs text-slate-500">{group.description}</p></div>
              </div>
              <span className={`w-fit rounded-full px-2.5 py-1 text-xs font-bold ${CATEGORY_STYLES[group.category]}`}>{groupDocuments.length}件</span>
            </div>
            {groupDocuments.length === 0
              ? <p className="mt-4 rounded-xl border border-dashed border-slate-200 bg-white px-4 py-5 text-center text-xs text-slate-400">この分類の書類はまだありません</p>
              : <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{groupDocuments.map((document) => <Link key={document.id} href={`?tab=${document.tab}`} className="group rounded-xl border border-slate-200 bg-white p-4 transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md">
                <div className="flex items-start gap-3"><span className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${CATEGORY_STYLES[document.category]}`}><FileText size={19} /></span><div className="min-w-0 flex-1"><div className="flex items-center justify-end"><span className="text-[10px] font-semibold text-slate-400">{document.status}</span></div><p className="mt-2 truncate font-bold text-slate-800 group-hover:text-blue-800">{document.name}</p><div className="mt-2 flex items-center justify-between gap-3 text-xs text-slate-400"><span>{dateLabel(document.date)}</span><span className="font-semibold tabular-nums text-slate-600">{document.detail}</span></div></div></div>
              </Link>)}</div>}
          </section>;
        })}</div>}
    </section>
