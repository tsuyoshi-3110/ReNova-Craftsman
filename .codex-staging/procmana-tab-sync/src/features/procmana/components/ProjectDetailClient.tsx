...
export function ProjectDetailClient({ projectId }: { projectId: string }) {
  const { member, demo } = useProcManaSession(); const search = useSearchParams(); const router = useRouter();
  const activeTab = normalizeProjectTab(search.get("tab"));
  ...
  const selectTab = (nextTab: ProjectTab) => {
    router.replace(`/procmana/projects/${projectId}?tab=${nextTab}`, { scroll: false });
  };
